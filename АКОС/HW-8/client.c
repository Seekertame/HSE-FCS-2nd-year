...
static void request_shutdown(void) {
    ...
    if (shared) {
        ...
        if (shared->server_pid > 0) {
            kill(shared->server_pid, SIGTERM);
        }
    }
}
...
int main(void) {
    ...
    printf("Client started with PID %d. Press Ctrl+C to stop.\n", getpid());
